import { AVATARS, POSTCARDS_LIMIT_PER_USER } from "@/utils/constants";
...
export const handleSubmit = async (
  ...
  const isUserCreatedPostCardMoreThanFive =
    userCreatedPostCardCount > POSTCARDS_LIMIT_PER_USER;

  if (isUserCreatedPostCardMoreThanFive) {
    notify(
      "warning",
      `You've reached the ${POSTCARDS_LIMIT_PER_USER} postcard limit. Delete one before creating a new one.`
    );
    return;
  }
